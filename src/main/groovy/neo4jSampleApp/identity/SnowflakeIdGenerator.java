package neo4jSampleApp.identity;
...
public class SnowflakeIdGenerator implements IdGenerator {
    ...
    public SnowflakeIdGenerator(long datacenterId) {
        if (datacenterId == 0) {
            try {
                ...
                if (log.isWarnEnabled()) {
                    log.warn("SNOWFLAKE: could not determine machine address; using random data-center Id");
                }
        ...
        if (log.isTraceEnabled()) {
            log.trace("SNOWFLAKE: initialized with data-center ID {}", this.datacenterId);
        }
